...
import datetime
import re
from PyQt6.QtWidgets import QTextBrowser,QApplication,QDialog, QMainWindow, QWidget, QVBoxLayout, QLabel, QHBoxLayout,QLineEdit, QGridLayout, QPushButton, QFrame,QMessageBox
...
class InputWithComboBox(QWidget):
    def __init__(self, options=None):
        ...
        self.combo_box.setFixedWidth(65)
...
class TimeInfoApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setGeometry(50, 50, 935, 780)  # 设置窗口大小和位置
        self.setWindowIcon(QIcon("TTB.jpg"))  # 替换成您的图标文件路径
        # 创建可滚动区域
        ...
        self.current_time_button.setFixedWidth(95)  # 设置标签的宽度为 100 像素
        ...
        # 创建一个 QLabel
        self.solar_label = QLabel("输入：<font color='red'>阳历</font> 年，月，日，时间(<font color='purple'>可查范围：1900年1月30日0时0分至2100年12月31日23时59分</font>)；以下部分为输出内容")
        # 设置对齐方式为居中
        ...
        for row in range(3):
            for col in range(3):
                ...
                if row==1 and col==1:
                    ...
                    output1.setFixedSize(157, 32)  # 设置宽度和高度
                    ...
                    output2.setFixedSize(157, 40)  # 设置宽度和高度
    ...
    def show_info(self):
        ...
        if ye == "" and mo == "" and da == "" and ho == "" and mi == "":
            ...
            reply.setText('施主，巧妇难为无米之炊，您得输入点东西我才能为你运算呐。')
        ...
        '''
                        ...
                        # 计算阴阳遁
                        YD = 'yangdun'
                        jieqiYangD = ["冬至", "小寒", "大寒", "立春", "雨水", "惊蛰", "春分", "清明", "谷雨", "立夏",
                                      "小满", "芒种"]
                        ...
                        if YD == 'yangdun':
                            ...
                            YD = '阴遁'
                        list1jiazi = ["甲子", "乙丑", "丙寅", "丁卯", "戊辰", "己巳", "庚午", "辛未", "壬申", "癸酉"]
                        list2jiaxu = ["甲戌", "乙亥", "丙子", "丁丑", "戊寅", "己卯", "庚辰", "辛巳", "壬午", "癸未"]
                        list3jiashen = ["甲申", "乙酉", "丙戌", "丁亥", "戊子", "己丑", "庚寅", "辛卯", "壬辰", "癸巳"]
                        list4jiawu = ["甲午", "乙未", "丙申", "丁酉", "戊戌", "己亥", "庚子", "辛丑", "壬寅", "癸卯"]
                        list5jiachen = ["甲辰", "乙巳", "丙午", "丁未", "戊申", "己酉", "庚戌", "辛亥", "壬子", "癸丑"]
                        list6jiayin = ["甲寅", "乙卯", "丙辰", "丁巳", "戊午", "己未", "庚申", "辛酉", "壬戌", "癸亥"]
                                        ...
                                        if len(sanqiliuyizhuangdonglist[
                                            ...
                                            self.output_boxes[
                                                dingweigongzhuangdong[sanqiliuyizhuangdongshuchugonghao][
                                                    4]].setPlainText(
                                                '%s' % (sanqiliuyizhuangdonglist[i - sanqiliuyilaodagonghaozhuandong]))
                                            self.output_boxes[
                                                dingweigongzhuangdong[sanqiliuyizhuangdongshuchugonghao][
                                                    4]].setAlignment(
                                                Qt.AlignmentFlag.AlignCenter)
                                            self.output_boxes[
                                                dingweigongzhuangdong[sanqiliuyizhuangdongshuchugonghao][4]].setFont(
                                                font)
                        ...
                        info = f"你输入的时间是：{user_time}"
                        self.lunar_output1.setText(' %s%s年(%s年,%s)''\t''%s%s月''\t''%s%s日''\t''%s%s时' % (
...
class LoginDialog(QDialog):
    def __init__(self):
        ...
        # 获取运行时的临时目录
        # 在类的构造函数中添加以下代码
        background_image = QPixmap("BJ.png")  # 替换为您的背景图像文件路径
        ...
        # 访问数据文件的路径
        image_path1 = os.path.join(base_path, 'HGXH2.png')
        image_path2 = os.path.join(base_path, 'QMDJ.png')
...
#登录部分
        ...
        self.username_input.setPlaceholderText("用户名(注册代码：LF2000)")
        ...
        spacer = QSpacerItem(20, 50, QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Fixed)
        ...
        spacer = QSpacerItem(20, 25, QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Fixed)
        login_layout.addItem(spacer)
        '''
        ...
        self.original_username = "LF"  # 初始用户名
        self.original_password = "200012171700"  # 初始密码
    ...
    def register(self):
        ...
        if ok and code in self.register_codes and self.register_codes[code]:
            ...
            self.login_status_label.setText("注册失败，注册代码错误或已被使用")
