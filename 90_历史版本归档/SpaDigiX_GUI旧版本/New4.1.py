...
'''
    ...
    def __init__(self):
        ...
        def replyyy():
            ...
            reply.setIcon(QMessageBox.Icon.NoIcon)
            reply.setWindowTitle("温馨提示")
            reply.setText('输入👉：<font color=red>阳历</font> 年，月，日，时，分(<font color=purple>可查范围：1900年2月1日0时0分至2100年12月31日23时59分</font>)。')
            reply.setStandardButtons(QMessageBox.StandardButton.Ok)
            reply.exec()  # 阻塞应用程序直到用户关闭警告框
    ...
    "大雪", "冬至", "小寒", "大寒"
        ...
        for entry in [self.NIANSHUCHUKUANG_shuru, self.YUESHUCHUKUANG_shuru, self.RISHUCHUKUANG_shuru, self.SHISHUCHUKUANG_shuru,self.JIEQIYDSHUCHU_shuru]:
            entry.input_text.setStyleSheet("border: 2px solid gray;")
            entry.input_text.setAlignment(Qt.AlignmentFlag.AlignCenter)
            entry.input_text.setFixedSize(90,25)
        # 阴历输出框的格式
        ...
        def replyyyy():
            ...
            reply.setWindowTitle("温馨提示")
            reply.setText(
                '输入👉：<font color=red>阴历</font> 年，月，日，时辰，节气(<font color=purple>注意两个历法只能选择一个输入，输入要完整，另一个需留白，阴历输入功能还未实现</font>)。')
            reply.setStandardButtons(QMessageBox.StandardButton.Ok)
            reply.exec()  # 阻塞应用程序直到用户关闭警告框
        ...
        for row in range(3):
            for col in range(3):
                ...
                if row == 1 and col == 1:
                    ...
                    output1 = QLineEdit()
                    output1.setStyleSheet("background-color: white; ")
                    output1.setAlignment(Qt.AlignmentFlag.AlignCenter)
                    ...
                    output3 = QLineEdit()
                    output3.setStyleSheet("background-color: white; ")
