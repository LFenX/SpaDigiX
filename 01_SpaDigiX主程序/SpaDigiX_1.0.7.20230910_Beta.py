...
'''
    ...
    def _show_info_legacy_reference(self):
        base_path = getattr(sys, '_MEIPASS', os.path.abspath(os.path.dirname(__file__)))
        # 访问数据文件的路径
        image_path1 = os.path.join(base_path, 'JGT.png')
    ...
    def _clear_chart_outputs_only(self):
        self.lunar_output1.clear()
        for output_box in self.output_boxes.values():
            output_box.clear()

    def _set_output_text(self, output_name, text):
        output_box = self.output_boxes[output_name]
        if hasattr(output_box, "setPlainText"):
            output_box.setPlainText(text)
        else:
            output_box.setText(text)
        output_box.setAlignment(Qt.AlignmentFlag.AlignCenter)

    def _render_chart_result(self, result):
        self._clear_chart_outputs_only()
        ...
        for output_name, text in result.output_boxes.items():
            self._set_output_text(output_name, text)

    def _parse_solar_inputs(self, ye, mo, da, ho, mi, image_path1):
        user_time = ye + '年' + mo + '月' + da + '日' + ho + '时' + mi + '分'
        pattern = re.compile(r'(\d{0,5})年(\d{1,3})月(\d{1,3})[号日]?(\d{1,3})[点时]?(\d{1,3})分?')
        matches = pattern.search(user_time)
        if not matches:
