...
# Load the Excel file
file_path_source = '定位、定级验证灵数误差表初始.xlsx'
...
data_error_1_source = pd.read_excel(file_path_source, sheet_name='澳门定位误差', parse_dates=['日期'], date_parser=pd.to_datetime)
data_error_2_source = pd.read_excel(file_path_source, sheet_name='香港定位误差', parse_dates=['日期'], date_parser=pd.to_datetime)
data_error_1_source_abs = pd.read_excel(file_path_source, sheet_name='澳门定位误差ABS', parse_dates=['日期'], date_parser=pd.to_datetime)
data_error_2_source_abs = pd.read_excel(file_path_source, sheet_name='香港定位误差ABS', parse_dates=['日期'], date_parser=pd.to_datetime)
...
output_file_path_updated_source = '1228输出_2023双干定位_新灵数误差表_第四版修改.xlsx'
...
# 读取澳门和香港的定位误差表
# 读取澳门和香港的定位误差表
data_error_1_source = pd.read_excel(output_file_path_updated_source, sheet_name='澳门定位误差', parse_dates=['日期'], date_format='%Y-%m-%d')
data_error_2_source = pd.read_excel(output_file_path_updated_source, sheet_name='香港定位误差', parse_dates=['日期'], date_format='%Y-%m-%d')
print(data_error_1_source)
print(data_error_2_source)
# 定义误差范围
error_ranges = [(-1,1),(-2,2),(-3,3),(-3, -2), (-2, -1), (-1, 0),(0,1), (1, 2), (2, 3)]

def calculate_error_percentage(data_error_source, time_period):
    total_count = len(data_error_source)
    percentages = []

    for error_range in error_ranges:
        count_within_range = data_error_source[
            (data_error_source[time_period + '误差'] >= error_range[0]) & (data_error_source[time_period + '误差'] <= error_range[1])
        ].shape[0]
...
print("澳门所有双干误差比例：")
for time_period in time_dict.keys():
    percentages = calculate_error_percentage(data_error_1_source, time_period)
    print(f"{time_period}误差在范围内的比例：")
    for i, error_range in enumerate(error_ranges):
        print(f"{error_range[0]}到{error_range[1]}之间的比例为：{percentages[i]:.2f}%")

# 打印香港所有双干误差在不同范围内的比例
print("\n香港所有双干误差比例：")
for time_period in time_dict.keys():
    percentages = calculate_error_percentage(data_error_2_source, time_period)
    print(f"{time_period}误差在范围内的比例：")
    for i, error_range in enumerate(error_ranges):
        print(f"{error_range[0]}到{error_range[1]}之间的比例为：{percentages[i]:.2f}%")
