...
def calculate_and_label(date):
    ...
    labell=getthebasicmessageofnineGrids(year, month, day, hourr)[1][2][1]
    return value, labell, nianyueriganzhi
...
for sheet_name, data in sheets_data.items():
    ...
    summary_data.append({'干支月': sheet_name, '误差均值': error_mean, '准确率': accuracy_rate})

# 创建汇总数据表
summary_df = pd.DataFrame(summary_data)

# 输出文件路径
output_file_path = '240108_2023定位香港双干十二月份结果P0.xlsx'
...
with pd.ExcelWriter(output_file_path, engine='xlsxwriter') as writer:
    ...
    # 将汇总数据表写入新的工作表
    summary_df.to_excel(writer, sheet_name='十二月份误差汇总', index=False)
    worksheet = writer.sheets['十二月份误差汇总']