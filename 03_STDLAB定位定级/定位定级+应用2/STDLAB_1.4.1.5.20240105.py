...
def calculate_score(row):
    ...
    '''
    if  dizhi in ["子", "丑", "寅", "卯", "辰", "巳"]:
        zhuangjia_number = 0
    else:
        zhuangjia_number = 1
    print(f"日期：{year}-{month}-{day}-{hour},地支: {dizhi},庄家数：{zhuangjia_number}")
    print("----------------------------------------------------------------------")
    return zhuangjia_number
# 读取 Excel 文件中的所有工作表
...
result_df.to_excel('统一地支-澳门2021-2023庄家模型数据汇总240105.xlsx', index=False)