'''
双干/1/14
***相较于上一个版本SpaDigiX_1.1.3.20231125_Alpha对以下几个部分进行了修改***
1.删去了吉格中的 乙+甲子戊（开门）：与后面发生重复
2.修正了 丙（生门）十九天-1 ：人为错误
3.去掉了五假： 用的较少
4.去掉了三奇入墓和时干入暮：与长生十二宫重复
5.其余格 乙+戊一概-1，不考虑门凶门迫
6.丙+辛 改为+1：人为错误
7.辛+丙 一律为+1，不考虑门吉
8.壬+乙 改为+1：人为错误
9.天盘干定数 与月份，与九宫，天盘干与地盘干五行力量 如果地盘干有两个，则最终得数要除2
10.九星力量暂不考虑吉凶星变化
11.八门力量中的特殊门遇到天盘干地盘干的力量变化按照定级的做法来
12.八神在午月中遇到值符时改为+0
13.甲子戊一律看为戊
14.长生12宫修改
'''
...
if len(zhishigongtianpan)==1:
    ...
    try:
        ...
        SpaDigiX_APPONE_Value = SpaDigiX_APPONE_Value + (tiandiwuxingliliangxiaozhangdingshu1 + tiandiwuxingliliangxiaozhangdingshu2 + tiandiwuxingliliangxiaozhangdingshu3 + tiandiwuxingliliangxiaozhangdingshu4)/4
