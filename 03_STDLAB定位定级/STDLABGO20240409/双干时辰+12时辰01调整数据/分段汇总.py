...
# 初始化一个字典来保存每个分类在所有时间点中的最高吻合度行
best_matches = {name: {"time": "", "data": None} for name in classification_names}

# 读取每个时间点的吻合度结果汇总表格，找到每个分类的最高吻合度
for time_point in time_points:
    summary_file_path = f'{time_point}-吻合度结果汇总.xlsx'
    summary_df = pd.read_excel(summary_file_path, sheet_name="最高吻合度")

    for classification_name in classification_names:
        # 找到当前时间点中当前分类的最高吻合度行
        current_row = summary_df[summary_df["工作表名称"] == classification_name]
        if not current_row.empty:
            current_match_rate = float(current_row["吻合度最高"].str.split(": ").str[-1].values[0])
            if (best_matches[classification_name]["data"] is None or
                    current_match_rate > float(
                        best_matches[classification_name]["data"]["吻合度最高"].str.split(": ").str[-1].values[0])):
                best_matches[classification_name]["time"] = time_point
                best_matches[classification_name]["data"] = current_row.iloc[0]

# 创建一个新的Excel文件来保存每个分类在所有时间点中的最高吻合度行
output_summary_path = '分段最高吻合度结果汇总.xlsx'
with pd.ExcelWriter(output_summary_path) as writer:
    summary_df = pd.DataFrame([best_matches[name]["data"] for name in classification_names])
    summary_df.to_excel(writer, sheet_name='最高吻合度', index=False)

# 从对应的澳门双干宫+阴阳遁分类结果中提取相应的工作表并将其放入新的Excel表格中
output_classification_path = '最高吻合度分类数据统计结果.xlsx'
with pd.ExcelWriter(output_classification_path) as writer:
    for classification_name in classification_names:
        time_point = best_matches[classification_name]["time"]
        classification_file_path = f'{time_point}-澳门双干宫+阴阳遁分类结果+最高吻合度列数据.xlsx'
