...
def calculate_fill_rate_with_debt(S, demand_data, distributors):
    ...
    for period in range(num_periods):
        ...
        debt = {d: max(0, distributors[d]['beta'] * distributors[d]['mean'] * (period + 1) - np.sum(
            total_supply[d][:period])) for d in distributors}
...
while S_upper - S_lower > 1:
    ...
    if len(allocation_orders) == 0 or allocation_orders[-1] != allocation_order:
...
for period, orders in enumerate(allocation_orders):
    allocation_orders_flattened.append({'周期': period, '分配顺序': ','.join([','.join(order) for order in orders])})
...
for period, alloc_result in enumerate(allocation_results_all[-1]):
    for d, amount in alloc_result.items():
...
with pd.ExcelWriter("bisection_process_detailed_v7.xlsx") as writer:
...
file_path = "bisection_process_detailed_v7.xlsx"
file_path
