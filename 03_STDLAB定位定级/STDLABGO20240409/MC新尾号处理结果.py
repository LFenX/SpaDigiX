...
# 创建一个新的 Excel 写入器
with pd.ExcelWriter('澳门新尾号处理结果.xlsx') as writer:
    # 遍历所有工作表
    for sheet_name in sheet_names:
        ...
        if indexx!=0:
            ...
            for i in range(7):  # 对于七个预测值
                ...
                df[f'下{i + 1}'] =[prediction2[i]] +[pred[i] for pred in predictions]  # 将所有的预测值包括最后一天
            ...
            for i in range(7):  # 对于七个预测值
                # 确保 predictions 的长度匹配 df 的行数
                df[f'下{i + 1}'] = [None] + [pred[i] for pred in predictions]  # 将所有的预测值包括最后一天
