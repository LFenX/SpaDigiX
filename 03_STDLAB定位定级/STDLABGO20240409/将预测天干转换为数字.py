...
file_path = '香港2010-2024年排干预测数据-高.xlsx'
xls = pd.ExcelFile(file_path)

with pd.ExcelWriter('香港2010-2024年排干预测完整数据-高.xlsx') as writer:
    for sheet_name in xls.sheet_names:
        ...
        # 转换天干为数字并创建新列
        for i in range(1, 8):  # 特1到特10
            tian_gan_col = f'{i}号预测天干'
            df[f'{i}号预测号码'] = df[tian_gan_col].apply(
                lambda x: (
