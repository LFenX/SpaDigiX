...
def process_all_sheets(excel_path, sheet_names):
    accumulated_results = {
        ...
        #"天干": defaultdict(lambda: defaultdict(lambda: defaultdict(int))),
        #"九星": defaultdict(lambda: defaultdict(lambda: defaultdict(int))),
        #"八门": defaultdict(lambda: defaultdict(lambda: defaultdict(int))),
        #"八神": defaultdict(lambda: defaultdict(lambda: defaultdict(int))),
        #"日在月": defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
    }
...
excel_file_path = '240523-进一步处理-澳门日在月调整更新结果.xlsx'  # Replace with your actual file path
...
output_accumulated_file_path = '澳门-日在月出现次数汇总240525.xlsx'
