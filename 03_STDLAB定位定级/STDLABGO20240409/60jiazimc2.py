...
def process_workbook(file_path, combined_data):
    ...
    for sheet_name in excel_data.sheet_names:
        ...
        #df['双干宫'] = df.apply(
            ...
            #axis=1)
        #df["年月日干支时间"]=df.apply( lambda row: getSpaDigiXdingweideshu(row['日期'].year, row['日期'].month, row['日期'].day)[2],
            #axis=1)

        for index, row in df.iterrows():
            #if pd.notnull(row['地支']):  # 检查生肖列是否有数据
                ...
                if digit_match and shengxiao_match:
                    ...
                    combined_value = digit_part + row['程序数']
    ...
    output_path = (f'加入生肖-{file_path}')
    with pd.ExcelWriter(output_path) as writer:
        for sheet_name, data in all_sheets_data.items():
            data.to_excel(writer, sheet_name=sheet_name, index=False)
    return output_path
file_paths = [
    '输入实数数据-加入程序数-加入时辰和生肖组香港10-24年原始数据.xlsx',
'输入实数数据-加入程序数-加入时辰和生肖组澳门20-24年原始数据.xlsx',
]
'''
file_paths = [
