...
# 读取上传的 Excel 文件
file_path = '含有对比01数据-输入生肖_输入实数数据-亥时-未输入生肖数据-澳门双干+阴阳遁分类结果-未输入对比时间01数据.xlsx'
...
output_path_with_lowest_cols = '亥时-澳门双干宫+阴阳遁分类结果+最低吻合度列数据.xlsx'
...
for sheet in sheet_names:
    ...
    non_match_count = sum(df[lowest_match_columns[0]] != df['对比01数据'])
    lowest_match_data.append({
        '工作表名称': sheet,
        '吻合度最低': f"{', '.join(lowest_match_columns)}: {lowest_match_rate}",
        '数据总数': len(df),
        '不吻合数据量': non_match_count
    })
...
output_path_summary = '亥时-最低吻合度结果汇总.xlsx'
