...
for sheet_name, df in excel_data.items():
    ...
    for col in ['上一', '上二', '上三', '上四', '上五', '上六', '上特', '公历日尾', '农历日尾']:
        df[f'{col}属性'] = df[col].map(attribute_map)

    # Add relationship columns
    for base_col in ['上一', '上二', '上三', '上四', '上五', '上六', '上特', '公历日尾', '农历日尾']:
        base_attr_col = f'{base_col}属性'
        relationships = []
        for idx, row in df.iterrows():
            relationship = []
            for compare_col in ['上一', '上二', '上三', '上四', '上五', '上六', '上特', '公历日尾', '农历日尾']:
                if base_col != compare_col:
                    compare_attr_col = f'{compare_col}属性'
                    rel = determine_relationship(row[base_attr_col], row[compare_attr_col])
                    relationship.append(rel)
            relationships.append('-'.join(relationship))
        df[f'{base_col}关系'] = relationships

    # Save back to dictionary
    excel_data[sheet_name] = df

# Save the processed data with relationships to a new Excel file
relationship_processed_file_path = '香港2010-2024年新尾号方法数据汇总.xlsx'
