def getdingjideshu(year,month,day):
    from functionSpadigiX_GetvaluefromFIVEthreeforG4MConelevel import gettheSpaDigiX_value
    ...
    for i in range(0, 12):
        ...
        if riganzhi == "甲子" and shichengganzhi == "甲子":
            ...
            if riganzhi[0] == shichengganzhi[0]:
                ...
                break
    # 双干
    shuanggandeshu = gettheSpaDigiX_value(year, month, day, hour, "双干", shuanggan)[0]
    shuanggangonghao = gettheSpaDigiX_value(year, month, day, hour, "双干", shuanggan)[1]
    jiaziwuindex=gettheSpaDigiX_value(year, month, day, hour, "双干", shuanggan)[2]
    # 值符
    zhifudeshu = gettheSpaDigiX_value(year, month, day, hour, "值符", shuanggan)[0]
    zhifugonghao = gettheSpaDigiX_value(year, month, day, hour, "值符", shuanggan)[1]
    # 值使
    zhishideshu = gettheSpaDigiX_value(year, month, day, hour, "值使", shuanggan)[0]
    zhishigonghao = gettheSpaDigiX_value(year, month, day, hour, "值使", shuanggan)[1]
    # 生门
    shengmendeshu = gettheSpaDigiX_value(year, month, day, hour, "生门", shuanggan)[0]
    shengmengonghao = gettheSpaDigiX_value(year, month, day, hour, "生门", shuanggan)[1]
    print(
        "-----------------------------------------------------------------------------------------------------------------")
    ...
    return SpaDigiX_dingji_value, shuanggandeshu, zhishideshu, zhifudeshu, shengmendeshu, nianyueriganzhi, riganzhi,shuanggangonghao,zhifugonghao,zhishigonghao,shengmengonghao,jiaziwuindex