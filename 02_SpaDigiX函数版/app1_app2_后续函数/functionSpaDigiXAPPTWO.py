from ephem import *
def getthefourmaxmessage(nian,yue,ri,zongtianshu,lianxutianshu,jiange,shengxiao,nianchaxun,yuechaxun,richaxun):
    ...
    L = lianxutianshu
    J = jiange
    dongwushengxiao =shengxiao
    Tiangan = ['甲', '乙', '丙', '丁', '戊', '己', '庚', '辛', '壬', '癸']
    Dizhi = ['子', '丑', '寅', '卯', '辰', '巳', '午', '未', '申', '酉', '戌', '亥']
    shengxiao = ['鼠', '牛', '虎', '兔', '龙', '蛇', '马', '羊', '猴', '鸡', '狗', '猪']
    qishi = datetime.datetime(yea, mon, da)
    tian = []
    yy = []
    zishi = ""
    choushi = ""
    ...
    xushi = ""
    haishi = ""
    shengxiaoshi = [zishi, choushi, yinshi, maoshi, chenshi, sishi, wushi, weishi, shenshi, youshi, xushi, haishi]
    XO = Z // (L + J)
    XOXO = Z - XO * J
    hh = {}
    for i in range(1, XOXO + 1):
        h = []
        dongwudingwei = i
        dayzhouqi = i // L
        daylianxu = i % L
        ...
        for j in range(0, 12):
                ...
                if sanqiliuyilaodagonghao == 4:
                    ...
                    if YD == '阳遁':
                        ...
                        for i in range(sanqiliuyilaodagonghaozhuandong,
                            ...
                            dingweigongzhuangdong[i % 8]["天盘"] = sanqiliuyizhuangdonglist[
            ...
            for i in range(0, 8):
                bamenjilu = dingweigongzhuangdong[i]["八门"]
                if "使" in bamenjilu:
                    zhishidingwei_index = i
                else:
                    continue
            for i in range(0, 8):
                shengmenjilu = dingweigongzhuangdong[i]["八门"]
                if "生" in shengmenjilu:
                    shengmendingwei_index = i
                else:
                    continue
            shitiangandizhi = shigan[0] + shigan[1]
            for i in range(0, 8):
                tianpanjilu = dingweigongzhuangdong[i]["天盘"]
                text = tianpanjilu.split("\n")
                if len(text) == 1:
                    if shichengtiangan == "甲":
                        if len(tianpanjilu) == 1:
                            continue
                        elif len(tianpanjilu) == 3:
                            tianpanjiluganzhi = tianpanjilu[0] + tianpanjilu[1]
                            if shichengganzhi == tianpanjiluganzhi:
                                shigandingwei_index = i
                        else:
                            continue
                    else:
                        if shichengtiangan == tianpanjilu[-1]:
                            shigandingwei_index = i
                        else:
                            continue
                else:
                    if shichengtiangan == "甲":
                        if len(text[0]) == 1:
                            textt = text[1][0] + text[1][1]
                            if textt == shichengganzhi:
                                shigandingwei_index = i
                            else:
                                continue
                        elif len(text[1]) == 1:
                            textt = text[0][0] + text[0][1]
                            if textt == shichengganzhi:
                                shigandingwei_index = i
                            else:
                                continue
                            ...
                            textt = text[0][0] + text[0][1]
                            texttt = text[1][0] + text[1][1]
                            if textt == shichengganzhi or texttt == shichengganzhi:
                                shigandingwei_index = i
                            else:
                                continue
                    else:
                        if len(text[0]) == 1:
                            textt = text[1][-1]
                            if textt == shichengtiangan or text[0] == shichengtiangan:
                                shigandingwei_index = i
                            else:
                                continue
                        elif len(text[1]) == 1:
                            textt = text[0][-1]
                            if textt == shichengtiangan or text[1] == shichengtiangan:
                                shigandingwei_index = i
                            else:
                                continue
                        else:
                            textt = text[0][-1]
                            texttt = text[1][-1]
                            if textt == shichengtiangan or texttt == shichengtiangan:
                                shigandingwei_index = i
                            else:
                                continue
            dongwushengxiao = dongwushengxiao.replace(" ", "")
            if len(dongwushengxiao) == 1:
                dongwushengxiaoxiuzheng = dongwushengxiao
            else:
                dongwushengxiaoxiuzheng = dongwushengxiao[dongwudingwei - 1]

            dongwugongdingwei = {"子": 0, "寅": 1, "丑": 1, "卯": 2, "辰": 3, "巳": 3, "午": 4, "未": 5, "申": 5,
                                 "酉": 6, "戌": 7, "亥": 7}
            for key in dongwugongdingwei:
                if dongwushengxiaoxiuzheng == key:
                    dongwugongdingwei_index = dongwugongdingwei[key]
                    break
                else:
                    continue
            dingwei = [shengmendingwei_index, zhishidingwei_index, shigandingwei_index, dongwugongdingwei_index]
            h.append(dingwei)
            shengmenru = ""
            shiganru = ""
            zhishiru = ""
            if shengmendingwei_index - dongwugongdingwei_index == 1 or shengmendingwei_index - dongwugongdingwei_index == -1 or shengmendingwei_index - dongwugongdingwei_index == 0 or shengmendingwei_index - dongwugongdingwei_index == 7 or shengmendingwei_index - dongwugongdingwei_index == -7:
                shengmenru = "生门"
            if shigandingwei_index - dongwugongdingwei_index == 1 or shigandingwei_index - dongwugongdingwei_index == -1 or shigandingwei_index - dongwugongdingwei_index == 0 or shigandingwei_index - dongwugongdingwei_index == 7 or shigandingwei_index - dongwugongdingwei_index == -7:
                shiganru = "时干"
            if zhishidingwei_index - dongwugongdingwei_index == 1 or zhishidingwei_index - dongwugongdingwei_index == -1 or zhishidingwei_index - dongwugongdingwei_index == 0 or zhishidingwei_index - dongwugongdingwei_index == 7 or zhishidingwei_index - dongwugongdingwei_index == -7:
                zhishiru = "值使"
            rightnowzifuchuang = f"{month}月{day}日"
            for i in range(0, 12):
                if Dizhi[i] == shichengdizhi:
                    shengxiaoshi[i] = shengxiaoshi[i] + shengmenru + shiganru + zhishiru + rightnowzifuchuang
                else:
                    continue
        hh[rightnow] = h
    # 定义固定词汇
    ...
    for idx, (string, name) in enumerate(zip(shengxiaoshi, names)):
        max_word = max(keywords, key=string.count)
        max_frequency = string.count(max_word)
        max_words = [word for word in keywords if string.count(word) == max_frequency]
        # 将最大频次词汇连接成一个字符串
        max_words_str = '、'.join(max_words)
        ppo.append(max_word)
        result[name] = {'最大频次词': max_words_str, '频次': max_frequency, '字符串': string}
    ...
    di_zhi_names = ["子时", "丑时", "寅时", "卯时", "辰时", "巳时", "午时", "未时", "申时", "酉时", "戌时", "亥时"]
    for i in range(0, 12):
        for j in oop:
            if j == ppo[i]:
                pass
            else:
                dictionary[di_zhi_names[i]].append(j)
    #print("有效天数：%s" % XOXO)
    #print("以下为排序结果(如果有并列结果，后面输出的日期默认为第一个结果的对应出现(有)或未出现(无)日期)：")
    ll = {}
    for key, value in sorted_result.items():
        count += 1
        daiquchu = value['字符串']
        for i in dictionary[key]:
            daiquchu = daiquchu.replace(i, "")
        keyword = value['最大频次词'][0] + value['最大频次词'][1]
        # 使用正则表达式查找指定词组后面紧邻的日期
        pattern = rf"{keyword}(\d+月\d+日)"
        matches = re.findall(pattern, daiquchu)
        matched_dates = []
        for match in matches:
            matched_dates.append(match)
        # 找到所有日期的正则表达式
        all_dates_pattern = r"\d+月\d+日"
        all_dates = re.findall(all_dates_pattern, daiquchu)
        # 找到未匹配的日期
        unmatched_dates = [date for date in all_dates if date not in matched_dates]
        if not matches:
            matches = ["无日期出现该结果"]
        if count <= 12:
            for n in range(0, 12):
                if key == di_zhi_names[n]:
                    for m in range(0, 3):
                        gg = value["最大频次词"][0] + value["最大频次词"][1]
                        if gg == keywords[m]:
                            ll[n] = m
                        else:
                            continue
                    ...
                    continue
        #print(f"{key}: {value['最大频次词']}({value['频次']})有----→{matches}无----→{unmatched_dates}")
        #if count == 4:
            #print("----------------------------------------------------------------------------")
    #print("----------------------------------------------------------------------------")
    #print("若要查询对应日期排名前四的时辰对应元素所在宫，请输入待查询时间的年月日信息")
    chaxunriqinian = nianchaxun
    chaxunriqiyue = yuechaxun
    chaxunriqiri = richaxun
    rightnow = datetime.datetime(chaxunriqinian, chaxunriqiyue, chaxunriqiri)
    h = []
    for j in range(0, 12):
        number_of_hours_to_add = 2 * j
        number_of_minutes_to_add = number_of_hours_to_add * 60
        ...
        dongwugongdingwei = {"子": 0, "寅": 1, "丑": 1, "卯": 2, "辰": 3, "巳": 3, "午": 4, "未": 5, "申": 5, "酉": 6,
                             "戌": 7, "亥": 7}
        for key in dongwugongdingwei:
            if dongwushengxiaoxiuzheng == key:
                dongwugongdingwei_index = dongwugongdingwei[key]
        ...
        h.append(dingwei)
    xx = []
    #print(f"您输入的查询时间为{chaxunriqinian}年{chaxunriqiyue}月{chaxunriqiri}日")
    for key, value in ll.items():
        jj = h[key][value]
        xx.append(jj)
    bagua_names = ["坎一", "艮八", "震三", "巽四", "离九", "坤二", "兑七", "乾六"]
    cout = 0
    chucunsiyuansusuozaigong=[]
    for key, value in sorted_result.items():
        cout += 1
        ccc=f"{key}: {value['最大频次词']}({value['频次']})---->{bagua_names[xx[cout - 1]]}宫"
        chucunsiyuansusuozaigong.append(ccc)
        #print(f"{key}: {value['最大频次词']}---->{bagua_names[xx[cout - 1]]}宫")
        if cout == 4:
            break
    hhhh="\n".join(chucunsiyuansusuozaigong)
    return hhhh

    # hh记录每天所有时辰四个元素所在宫，ll记录每个时辰最大频次元素(包括顺序）,xx记录每个时辰对应元素所在宫号
